		...
		if (isset($_POST["register"])) {
			if(isset($_POST["voornaam"]) && isset($_POST["achternaam"]) && isset($_POST["email"]) && isset($_POST["straatnaam"]) && isset($_POST["huisnummer"]) && isset($_POST["postcode"]) && isset($_POST["woonplaats"]) && isset($_POST["geslacht"]) && isset($_POST["wachtwoord"]) && isset($_POST["confirmwachtwoord"])){
				if($_POST["wachtwoord"] === $_POST["confirmwachtwoord"]){
					...
					header('location: ../../index.php?controller=register&action=Index&status=password');
				}
			}
		}
		PassCheck($this->canPass);
 }//END OF CONSTRUCT
}//END OF CLASS
...
function PassCheck($canPass){
	if($canPass){
		...
		if($insertclass->insert_user($sql, $gebruikersnaam, $voornaam, $achternaam, $tussenvoegsel, $date, $email, $straatnaam, $huisnummer, $postcode, $woonplaats, $geslacht, $rechten, $wachtwoord)){
			header('location: ../../index.php?controller=register&action=Index&status=success');
		}else{
			header('location: ../../index.php?controller=register&action=Index&status=fail');
		}
